...
import type { PracticeRepositoryItem } from "./practice-types";
...
function PracticeEmptyState({ keyword }: { keyword: string }) {
  return (
    <MotionReveal
      data-state="empty"
...
export function PracticeRepositoryList({
  items,
  loading,
  error,
  keyword,
  ...
  if (error) {
    return <PracticeErrorState error={error} onRetry={onRetry} />;
  }

  if (!items.length) {
    return <PracticeEmptyState keyword={keyword} />;
  }
