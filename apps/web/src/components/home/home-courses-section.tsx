    ...
    >
      <MotionReveal className="grid gap-4" direction="left">
        ...
        <div className="grid gap-6 lg:grid-cols-12">
          <article className="home-panel overflow-hidden rounded-3xl border border-border lg:col-span-5">
            <div className="home-panel-strong overflow-hidden p-4">
              <div className="overflow-hidden rounded-2xl border border-border/60 bg-muted">
                {resolveMediaUrl(String(featuredCourse.cover ?? "")) ? (
                  ...
                  <div className="flex aspect-16/10 items-center justify-center p-6">
                    <div className="rounded-full bg-card px-4 py-2 text-[11px] font-bold tracking-[0.16em] text-primary shadow-sm">
                      课程聚焦
                    </div>
                  </div>
                )}
            ...
            <div className="space-y-5 p-6">
              <div className="space-y-3">
                <Badge className="w-fit rounded-md bg-accent px-2.5 py-1 text-[10px] font-bold text-primary hover:bg-accent">
                  推荐课程
                </Badge>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="home-panel-muted rounded-2xl border border-border/60 px-4 py-4">
              ...
              <div className="flex flex-wrap gap-3">
                <Link
                  ...
                  className="inline-flex flex-1 items-center justify-center rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground transition hover:brightness-95"
                >
                  进入课程
                </Link>
                <Link
                  href="/courses"
                  className="home-panel-muted home-interactive inline-flex flex-1 items-center justify-center rounded-xl border border-border px-5 py-3 text-sm font-bold text-foreground transition hover:bg-muted"
                >
                  全部课程
                </Link>
              </div>
          ...
          >
            {secondaryCourses.map((item, index) => (
              <MotionItem key={index}>
                <article className="home-panel home-interactive group cursor-pointer rounded-2xl border border-border p-5 hover:shadow-md">
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-2">
                      <span className="rounded-md bg-accent px-2 py-1 text-[10px] font-bold text-primary">
                        推荐课 {index + 2}
                      </span>
          ...
          <div className="grid gap-4 lg:col-span-3 flex-col">
            <section className="home-panel rounded-2xl border border-border p-5">
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="rounded-md border border-[var(--home-accent-border)] bg-accent px-2 py-1 text-[10px] font-bold text-primary">
                  实时
                </span>
              </div>
              <div className="mt-4">
              ...
              <div className="mt-4 grid gap-3">
                {visibleAnnouncements.map((item, index) => (
                  <div
                    key={index}
                    className="home-panel-muted rounded-2xl border border-border/60 px-4 py-4"
                  >
                    <div className="flex items-start gap-3">
                      <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full bg-accent text-xs font-bold text-primary">
                        {index + 1}
                      </div>
            ...
            <div className="home-panel-emphasis rounded-2xl border border-border p-5">
              <p className="text-xs font-bold text-muted-foreground">
                学习进度
              </p>
              ...
              <div className="home-progress-track mt-4 h-2 overflow-hidden rounded-full">
                <div className="h-full w-[84%] rounded-full bg-primary" />
              </div>
            </div>
            <div className="home-primary-panel rounded-2xl p-5">
              <p className="text-xs font-bold text-[var(--home-primary-panel-muted)]">近期考试</p>
              <p className="mt-4 text-4xl font-extrabold tracking-tight">
                {examCount || 6}
              </p>
              <p className="mt-2 text-[10px] font-bold text-[var(--home-primary-panel-muted)]">
                待关注场次
              </p>
            </div>
            <MotionStagger
            ...
            >
              {[
                ...
                <MotionItem key={item.label}>
                  <Link
                    href={item.href}
                    className="home-panel home-interactive group block rounded-xl border border-border p-3 text-center transition hover:bg-muted"
                  >
